...
export default function WriteStamp({card, openModal}) {
  ...
  const loadPost = async () => {
    const response = await axios.post('http://ec2-3-37-76-88.ap-northeast-2.compute.amazonaws.com:8000/api/stampcheck'
    , {id, m_num, poster}
    );
  ...
  useEffect(()=> {
    ...
    const handleSubmit = (e) => {
      ...
      if (img.file == null) {
        ...
        axios.post("http://ec2-3-37-76-88.ap-northeast-2.compute.amazonaws.com:8000/api/writestamp", formData, config)
        .then((response) => {
          if (part == 1) {
    ...
    const clickPoster1 = (e) => {
      ...
      if (record1 != true) {
        ...
        const res = axios.post("http://ec2-3-37-76-88.ap-northeast-2.compute.amazonaws.com:8000/api/stampcheck", {
          id,
          m_num,
          poster,
          part
