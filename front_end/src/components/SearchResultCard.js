...
export default function SearchResultCard({card}) {
  ...
  const loadPost = async () => {
    const response = await axios.post('http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com:8000/api/post/likelistcheck'
    , {id, l_num}
    );
  ...
  const loadImage = async () => {
    const response = await axios.get('http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com:8000/api/locationimage');
  ...
  const addLikelist = () => {
    ...
    if (id == undefined) return alert("로그인 후 이용 가능합니다."), document.location.href = '/login';
      ...
      axios.post('http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com:8000/api/post/likelistcheck',{id, l_num})
        .then(function (response) {
          console.log(response)
          if (response.data[0] == undefined) return (
            axios.post('http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com:8000/api/post/likelist',{id, l_num})
            .then(function (response) {
              alert("즐겨찾기에 추가되었습니다")
              setLike(true);
              })
            ...
            axios.post('http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com:8000/api/post/deletelikelist',{id, l_num})
            .then(function (response) {
