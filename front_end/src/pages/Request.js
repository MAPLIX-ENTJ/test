...
const Request = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (media_name == "") {
      ...
      if (img.file == null){
        axios.post("http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com/api/mypage/request", {media_name, r_content, id, m_type})
        .then((response) => {
        ...
        axios.post("http://ec2-3-38-107-72.ap-northeast-2.compute.amazonaws.com/api/mypage/requestimg", formData, config)
        .then((response) => {
          console.log(response);
          alert("요청 완료")
          document.location.href = '/'
